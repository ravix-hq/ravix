...
test('iOS accessibility coordinates use exact labels and bounded point frames', () => {
  ...
  expect(iosStartupAction(JSON.stringify(['Ravix Hello','Reload','Go home','Close'].map(node)))).toEqual({x:60,y:50});
});
test('iOS startup opens only the named Hello development-client confirmation', () => {
  ...
  for (const title of ['Open in “Ravix Hello”?', 'Open in "Ravix Hello"?']) {
  ...
  expect(iosStartupAction(hierarchy('Ravix Hello', 'Cancel', 'Open'))).toBeNull();
});
test('preview platform is explicit while older Android pairing files remain valid', () => {
  const base={expectedAccount:'ravix',buildDirectory:'/Users/ravix/.local/share/ravix/builds/experiment-52e8255f-b89c-4596-846d-1aa6d6002041',artifactSha256:'a'.repeat(64),serverUrl:'https://app.ravix.sh',pairingCode:'p'.repeat(43)};
