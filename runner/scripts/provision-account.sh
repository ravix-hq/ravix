...
companion_source="$source_home/.local/share/ravix/tools/idb-companion/1.1.8"
...
stage="$(mktemp -d /private/tmp/ravix-provision.XXXXXXXX)"
...
if [[ "$setup_mode" == --pair-runner ]]; then
  runner_config=/private/tmp/ravix-runner.json
  [[ -f "$runner_config" && ! -L "$runner_config" && "$(stat -f %u "$runner_config")" == "$(id -u "$SUDO_USER")" ]] || {
    echo 'Prepare /private/tmp/ravix-runner.json with the verified builds first.' >&2; exit 1
  }
  /usr/bin/install -m 600 "$runner_config" "$stage/runner-config.json"
  echo 'Paste the runner pairing code from Ravix:'
...
if [[ "$setup_mode" == --run-hello || "$setup_mode" == --preview-ios-hello || "$setup_mode" == --preview-hello ]]; then
  runtime_config=/private/tmp/ravix-hello-runtime.json
  if [[ "$setup_mode" == --preview-hello ]]; then runtime_config=/private/tmp/ravix-hello-preview.json; fi
  if [[ "$setup_mode" == --preview-ios-hello ]]; then runtime_config=/private/tmp/ravix-hello-ios-preview.json; fi
...
if [[ "$setup_mode" == --build-hello || "$setup_mode" == --build-ios-hello ]]; then
  fixture_snapshot=/private/tmp/ravix-hello-source.json
...
runtime="$runner_home/.local/share/ravix"
...
if [[ "$setup_mode" == --run-hello || "$setup_mode" == --preview-ios-hello || "$setup_mode" == --preview-hello ]]; then
  [[ -x "$local_bin/bun" && -x "$local_bin/ravix-runner" ]] || {
...
if [[ "$setup_mode" == --build-hello || "$setup_mode" == --build-ios-hello ]]; then
  ...
  "$local_bin/bun" -e '
    ...
    await Bun.write(stage + "/build.json", JSON.stringify({ snapshot: stage + "/source.json", stateDirectory: home + "/.local/share/ravix/builds", expectedAccount: account, platform: mode === "--build-ios-hello" ? "ios" : "android" }));
...
if [[ "$setup_mode" == --check ]]; then
  [[ -x "$local_bin/ravix-runner" && -f "$runtime/runner/index.ts" ]] || {
    echo 'Runner installation is missing; complete installation first.' >&2; exit 1
  }
...
cat > "$local_bin/ravix-runner" <<'LAUNCHER'
#!/bin/sh
export PATH="$HOME/.local/bin:/opt/homebrew/bin:/usr/bin:/bin:/usr/sbin:/sbin"
exec "$HOME/.local/bin/bun" "$HOME/.local/share/ravix/runner/index.ts" "$@"
LAUNCHER
chmod 700 "$local_bin/ravix-runner"
fi
echo 'Checking runner tools (emulator probes run sequentially, up to 60 seconds each)...'
check_status=0
"$local_bin/ravix-runner" doctor > "$runtime/doctor.json" || check_status=$?
"$local_bin/bun" -e '
