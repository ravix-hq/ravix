...
if (!executablePath) throw Error("Set RAVIX_BROWSER_TEST_EXECUTABLE to an installed Chromium executable.");
...
const server = Bun.serve({ port, hostname: "127.0.0.1", fetch(req) {
  ...
  return new Response('<!doctype html><meta name="viewport" content="width=device-width, initial-scale=1"><title>Browser proof</title><link rel="stylesheet" href="/app.css"><div id="root"></div><script type="module" src="/app.js"></script>', { headers: { "content-type": "text/html", "set-cookie": "ravix_session=local-proof; Path=/; HttpOnly; SameSite=Lax" } });
} });
