...
async function fixture() {
  ...
  for (const user of [owner, guest]) await db.createSession(user.id, await sha256(user.login), 60_000);
  await db.previews.setDefaults("p", { directory: "apps/demo", command: 'npm run dev -- --port "$PORT" --strictPort', readinessPath: "/health" });
  ...
  cleanup.push(() => { manager.stop(); void ctx.db.close(); upstream.stop(true); rmSync(dir, { recursive: true, force: true }); });
  return { ctx, sql, state, manager, request, owner, guest };
}
...
test("parallel starts allocate different ports and repeated opens are idempotent", async () => {
  ...
  const a = (await f.ctx.db.previews.get("t1"))!, b = (await f.ctx.db.previews.get("t2"))!;
  ...
  expect(f.state.services.get(`${b.sprite}/${b.service}`)).toBe("running"); expect((await f.ctx.db.previews.get("t2"))!.state).toBe("ready");
});

test("allocation remains unique across SQLite connections", async () => {
  const f = await fixture(); const second = await Db.open(f.sql);
  try {
    const a = await f.ctx.db.previews.allocate("t1", "s1", "sprite"); const b = await second.previews.allocate("t2", "s1", "sprite");
    expect(a.port).not.toBe(b.port);
    await expect(second.previews.save({ ...b, port: a.port })).rejects.toThrow();
...
test("a stop during service creation rejects the stale readiness result", async () => {
  ...
  expect((await f.ctx.db.previews.get("t1"))!.state).toBe("stopped");
...
test("configuration change during startup cannot publish an old service as Ready", async () => {
  ...
  expect((await f.manager.info("t1")).state).toBe("stopped"); expect((await f.manager.info("t1")).config!.directory).toBe("app2");
  await f.manager.startService("t1"); expect((await f.manager.info("t1")).state).toBe("ready");
});
...
test("port collisions and repeated crashes fail with logs, without an endless supervisor restart loop", async () => {
  ...
  await f.manager.startService("t1"); expect((await f.manager.info("t1")).error).toContain("Port occupied"); expect(f.state.creates).toBe(0);
  f.state.collide = false; f.state.crash = 3;
  await f.manager.startService("t1"); expect((await f.manager.info("t1")).state).toBe("failed"); expect((await f.manager.info("t1")).logs).toContain("command not found");
...
test("changing or restarting a stopped service replaces the provider definition instead of accepting its no-op PUT", async () => {
  ...
  const peer = (await f.ctx.db.previews.get("t2"))!;
  await f.manager.stopService("t1");
  ...
  expect((await f.manager.info("t1")).state).toBe("ready");
  expect((await f.manager.info("t1")).logs).not.toContain("already running");
  ...
  const row = (await f.ctx.db.previews.get("t1"))!;
  await f.ctx.db.previews.save({ ...row, logs: "old startup response" });
  await f.manager.stopService("t1"); await f.manager.startService("t1");
  expect((await f.manager.info("t1")).logs).not.toContain("old startup response");
...
test("restart reconciliation recovers saved intent, replaces sandboxes, and retries an interrupted stop", async () => {
  ...
  const original = (await f.ctx.db.previews.get("t1"))!;
  f.state.services.clear(); await f.ctx.db.close(); f.ctx.db = await Db.open(f.sql);
  await f.manager.tick(); expect((await f.manager.info("t1")).state).toBe("ready"); expect(f.state.creates).toBe(2);
  f.state.sandbox = "s2"; await f.manager.tick(); expect((await f.ctx.db.previews.get("t1"))!.sprite).toBe("s2");
  ...
  expect((await f.ctx.db.previews.get("t1"))!.stopPending).toBe(true);
  f.state.failStop = false; await f.manager.tick(); expect((await f.ctx.db.previews.get("t1"))!.stopPending).toBe(false);
});
...
test("lease expiry does not health-poll an idle machine; idle stop and closed-track cleanup preserve peers", async () => {
  ...
  let row = (await f.ctx.db.previews.get("t1"))!; await f.ctx.db.previews.save({ ...row, leaseUntil: Date.now() - 1 });
  const before = f.state.reads;
  // Isolate t1's idle behavior; t2 remains running but its lease expires too.
  const b = (await f.ctx.db.previews.get("t2"))!; await f.ctx.db.previews.save({ ...b, leaseUntil: Date.now() - 1 });
  await f.manager.tick(); expect(f.state.reads).toBe(before);
  row = (await f.ctx.db.previews.get("t1"))!; await f.ctx.db.previews.save({ ...row, lastActivity: Date.now() - 6 * 60_000 });
  await f.manager.tick(); expect((await f.manager.info("t1")).state).toBe("stopped"); expect((await f.manager.info("t2")).state).toBe("ready");
  await f.manager.startService("t1"); expect((await f.manager.info("t1")).state).toBe("ready");
  await f.ctx.db.closeTrack("t1"); await f.manager.tick(); expect((await f.ctx.db.previews.get("t1"))!.port).toBeNull();
...
test("a sandbox replacement during readiness cannot publish the stale service as Ready", async () => {
  ...
  await f.manager.startService("t1");
  expect((await f.manager.info("t1")).state).toBe("failed"); expect((await f.manager.info("t1")).error).toContain("workspace changed");
  expect([...f.state.services.values()]).toEqual(["stopped"]);
});

test("a running process is not Ready until the HTTP readiness path succeeds", async () => {
  const f = await fixture(); let probes = 0;
  f.manager.ready = async () => { probes++; if (probes === 1) { expect((await f.manager.info("t1")).state).toBe("starting"); return false; } return true; };
  await f.manager.startService("t1"); expect(probes).toBe(2); expect((await f.manager.info("t1")).state).toBe("ready");
});
...
test("overlapping restart requests discard superseded operations and leave the latest intent running", async () => {
  ...
  expect(f.state.creates).toBe(2); expect((await f.manager.info("t1")).state).toBe("ready");
...
async function agentFixture(guest = false) {
  const f = await fixture();
  if (guest) await f.ctx.db.addMember("t1", f.guest.id, f.owner.id);
  const user = guest ? f.guest : f.owner;
  const prompt = await f.ctx.db.enqueuePrompt({ id: crypto.randomUUID(), trackId: "t1", userId: user.id, authorLogin: user.login, payload: JSON.stringify({ prompt: "Set up preview", images: [] }) });
  await f.ctx.db.claimPrompt(prompt.id);
...
test("an installed agent helper configures and starts only its track without granting browser access", async () => {
  ...
  expect((await f.manager.info("t1")).config).toEqual(config);
  expect((await f.ctx.db.previews.defaults("p"))!.directory).toBe("apps/demo");
  const start = await f.call("start"); expect(start.status).toBe(200);
  expect((await start.json()).data.openUrl).toBeUndefined();
  for (let i = 0; i < 100 && (await f.manager.info("t1")).state !== "ready"; i++) await Bun.sleep(5);
  expect((await f.manager.info("t1")).state).toBe("ready");
  ...
  expect((await f.manager.info("t1")).override).toBeNull();
});

test("agent capabilities expire, rotate, and remain revoked after removal and reinvitation", async () => {
  const f = await agentFixture(true);
  const hash = await sha256(f.token), grant = (await f.ctx.db.previews.agentGrant(hash))!;
  await f.ctx.db.previews.grantAgent({ ...grant, expires: Date.now() - 1 });
  expect((await f.call("status")).status).toBe(401);
  await f.ctx.db.previews.grantAgent({ ...grant, expires: Date.now() + 60_000 });
  await f.ctx.db.removeMember("t1", f.guest.id); await f.ctx.db.addMember("t1", f.guest.id, f.owner.id);
...
test("agent helper rejects cancelled turns, replacement conversations, closed tracks, and changed sandboxes", async () => {
  ...
  await cancelled.ctx.db.setPromptStatus(cancelled.prompt.id, "cancelled"); expect((await cancelled.call("status")).status).toBe(401);
  const f = await agentFixture();
  await f.ctx.db.setPromptStatus(f.prompt.id, "sent");
  await f.ctx.db.attachConversation("t1", "replacement"); expect((await f.call("status")).status).toBe(401);
  await f.ctx.db.attachConversation("t1", "t1");
  f.state.sandbox = "s2"; expect((await f.call("configure", null)).status).toBe(409);
  f.state.sandbox = "s1"; await f.ctx.db.closeTrack("t1"); expect((await f.call("status")).status).toBe(409);
});
...
test("helper grants survive SQLite reopen but are revoked by project removal and track cleanup", async () => {
  ...
  await f.ctx.db.addProjectMember("p", f.guest.id, f.owner.id);
  await f.ctx.db.removeProjectMember("p", f.guest.id);
  expect((await f.call("status")).status).toBe(401);
