...
test('registered Mac takes queued tracks in order without another browser pairing', async()=>{
    const f=await fixture();
    const caps={version:1,capacity:{sessions:1,builds:1},builds:[{platform:'android',architecture:'arm64-v8a',profile:'pixel_7',runtime:'android-35',toolchain:'sdk35',artifactSha256:APK,sourceDigest:'b'.repeat(64),lockfileDigest:'c'.repeat(64)}]};
    expect((await f.request('/api/tracks/track/native/runner-pair','POST',{},'member')).status).toBe(403);
    const pairing=(await (await f.request('/api/tracks/track/native/runner-pair','POST')).json()).data;
    const register=()=>fetch(`http://127.0.0.1:${f.port}/api/native/runners/register`,{method:'POST',body:JSON.stringify({code:pairing.code,name:'Test Mac',capabilities:caps})});
    const response=await register();expect(response.status).toBe(200);const identity=(await response.json()).data;
    expect((await register()).status).toBe(401);
    const Client=WebSocket as unknown as new(url:string,opts:{headers:Record<string,string>})=>WebSocket;
    const host=new Client(`ws://127.0.0.1:${f.port}/api/native/runners/${identity.id}/control`,{headers:{authorization:`Bearer ${identity.token}`}});
    cleanup.push(()=>host.close());
    const messages:any[]=[];host.onmessage=e=>{const m=JSON.parse(String(e.data));messages.push(m);if(m.type==='connected')host.send(JSON.stringify({type:'hello',owned:null}));};
    await until(()=>messages.some(m=>m.type==='heartbeat'));
    const reqId=crypto.randomUUID(),secondId=crypto.randomUUID();
    const first=(await (await f.request('/api/tracks/track/native/start','POST',{platform:'android',requestId:reqId})).json()).data;
    expect(first.pairingCode).toBeUndefined();
    expect((await (await f.request('/api/tracks/track/native/start','POST',{platform:'android',requestId:reqId})).json()).data.id).toBe(first.id);
    const second=(await (await f.request('/api/tracks/other/native/start','POST',{platform:'android',requestId:secondId})).json()).data;
    expect(second.phase).toBe('Queued');
    await until(()=>messages.some(m=>m.type==='work'));
    const work=messages.find(m=>m.type==='work').work;
    expect(work.sessionId).toBe(first.id);
    host.send(JSON.stringify({type:'heartbeat',owned:{id:work.id,generation:work.generation}}));
    const claimed=await f.claim(work.pairingCode);expect(claimed.status).toBe(200);
    const native=await f.connect(first.id,'runner',(await claimed.json()).data.token);
    native.send(JSON.stringify({type:'heartbeat'}));
    await until(async()=>(await (await f.request('/api/tracks/track/native')).json()).data.session.phase==='Connecting');
    await f.request('/api/tracks/track/native/stop','POST');
    ...
    expect((await f.request(`/api/native/runners/${identity.id}/revoke`,'POST',{},'member')).status).toBe(404);
    expect((await f.request(`/api/native/runners/${identity.id}/revoke`,'POST')).status).toBe(200);
    await until(()=>host.readyState===WebSocket.CLOSED);
    expect((await f.claim(next.pairingCode)).status).toBe(401);
});
