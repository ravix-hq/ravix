...
interface Engine {
    ...
    info(id: string): Promise<NativeInfo | null>;
    busy(): Promise<boolean>;
}
const chains = new WeakMap<object, Promise<void>>();
/** Bun calls socket handlers synchronously and in order; now that they await the
 * database, each socket's events still run one at a time, in the order they arrived. */
export function serial(ws: object, fn: () => void | Promise<void>, what: string) {
    const next = (chains.get(ws) ?? Promise.resolve()).then(fn).catch(error => console.error(what + ':', error));
    chains.set(ws, next);
    return next;
}
...
export class RunnerCoordinator {
    ...
    start() { return this.ctx.db.runners.recover(); }
    private async enabled(projectId: string) { const p = await this.ctx.db.project(projectId); return !!this.ctx.config.nativePreviewExperiment && !!this.ctx.sprites && !!this.ctx.fountain && p?.repoFullName === 'ravix-hq/ravix-expo-hello' && !p.archivedAt; }
    async list(projectId: string): Promise<RunnerInfo[]> { return (await this.ctx.db.runners.runners()).filter(r => r.projects.includes(projectId)).map(r => ({ id: r.id, name: r.name, projects: [projectId], revoked: r.revoked, online: !r.revoked && this.peers.has(r.id) && Date.now() - r.lastSeen < RUNNER.leaseMs, capabilities: r.capabilities })); }
    async authorized(s: NativeRequest) {
        const user = await this.ctx.db.sessionUser(s.sessionHash), r = await this.ctx.db.runners.runner(s.runnerId), track = await this.ctx.db.track(s.trackId), project = await this.ctx.db.project(s.projectId);
        return !!user && user.id === s.userId && !!r && !r.revoked && r.owner === s.userId && r.projects.includes(s.projectId) && !!track && !track.closedAt && track.workdir === s.workdir && !!project && !project.archivedAt && project.userId === s.userId && project.rev === s.projectRevision && project.agentId === s.agentId && await this.enabled(project.id);
    }
    async current(id: string, generation: number, epoch: number) { const s = await this.ctx.db.runners.request(id), r = s && await this.ctx.db.runners.runner(s.runnerId); return !!s && !!r && await this.authorized(s) && s.desired === 'run' && s.generation === generation && s.epoch === epoch && r.epoch === epoch && s.leaseUntil > Date.now() && s.deadline > Date.now() && !['Queued', 'Reconciling', 'Stopping', 'Stopped', 'Failed'].includes(s.phase); }
    async info(s: NativeRequest): Promise<NativeInfo> {
        const actual = !['Queued', 'Reconciling'].includes(s.phase) ? await this.engine.info(s.id) : null;
        return { id: s.id, trackId: s.trackId, platform: s.platform, phase: s.phase, error: s.error, expiresAt: s.deadline, runnerOnline: (await this.list(s.projectId)).some(r => r.id === s.runnerId && r.online), video: actual?.video ?? null, frames: actual?.frames ?? 0, queuePosition: await this.ctx.db.runners.position(s.id), runnerId: s.runnerId, generation: s.generation };
    }
    async pair(req: Request, trackId: string) {
        const user = await authenticate(this.ctx, req), { project, role } = await trackAccess(this.ctx, user, trackId);
        ...
        if (!await this.enabled(project.id))
            throw new HttpError(501, 'native_unavailable');
        const code = randomToken();
        try {
            await this.ctx.db.runners.pair(await sha256(code), user.id, [project.id]);
        }
        catch (e) {
    ...
    async register(req: Request) {
        ...
        let runner;
        try {
            runner = await this.ctx.db.runners.register(await sha256(v.code), await sha256(token), v.name, capabilities);
        }
        catch {
            throw new HttpError(401, 'pairing_expired', 'Create a fresh runner pairing code.');
        }
        let every = true;
        for (const id of runner.projects)
            if (!(every = await this.enabled(id) && (await this.ctx.db.project(id))?.userId === runner.owner))
                break;
        if (!every) {
            await this.ctx.db.runners.revoke(runner.id);
    ...
    async revoke(req: Request, id: string) {
        const user = await authenticate(this.ctx, req), r = await this.ctx.db.runners.runner(id);
        if (!r || r.owner !== user.id)
            throw new HttpError(404, 'not_found');
        if (req.headers.get('origin') !== this.ctx.config.publicUrl)
            throw new HttpError(403, 'origin');
        await this.ctx.db.runners.revoke(id);
    ...
    enqueue(req: Request, trackId: string, platform: NativePlatform, requestId: string, runnerId?: string) {
        ...
        return (async () => {
            ...
            if (req.headers.get('origin') !== this.ctx.config.publicUrl)
                throw new HttpError(403, 'origin');
            if (!await this.enabled(project.id) || track.closedAt)
                throw new HttpError(409, 'native_unavailable');
            if (runnerId !== undefined && !uuid(runnerId))
                throw new HttpError(400, 'invalid_runner');
            const runner = (await this.list(project.id)).find(r => !r.revoked && (!runnerId || r.id === runnerId) && r.capabilities.builds.some(b => b.platform === platform));
            if (!runner)
            ...
            let s;
            try {
                s = await this.ctx.db.runners.enqueue({ trackId, projectId: project.id, userId: user.id, sessionHash: await sha256(sessionHash), projectRevision: project.rev, agentId: project.agentId, workdir: track.workdir }, runner.id, platform, requestId);
            }
            catch (e) {
    ...
    async stop(id: string) { await this.ctx.db.runners.stop(id); await this.tick(); }
    async upgrade(req: Request, server: Server<RunnerPeer>) {
        ...
        const token = /^Bearer ([\w-]{43})$/.exec(req.headers.get('authorization') ?? '')?.[1], r = await this.ctx.db.runners.runner(match[1]!);
        if (!token || !r || r.revoked || await sha256(token) !== r.tokenHash)
            return new Response('Unauthorized', { status: 401 });
        if ((await this.ctx.db.runners.runner(r.id))?.revoked)
            return new Response('Unauthorized', { status: 401 });
    ...
    readonly websocket = {
        open: (ws: ServerWebSocket<RunnerPeer>) => serial(ws, async () => {
            const prior = this.peers.get(ws.data.runnerId), r = await this.ctx.db.runners.connect(ws.data.runnerId);
        ...
        message: (ws: ServerWebSocket<RunnerPeer>, data: string | Buffer) => serial(ws, async () => {
            try {
                ...
                if (!await this.ctx.db.runners.heartbeat(ws.data.runnerId, ws.data.epoch))
                ...
                if (v.type === 'hello' || v.type === 'heartbeat') {
                    ...
                    if (v.owned) {
                        const s = (await this.ctx.db.runners.requests()).find(s => s.jobId === v.owned.id);
                        if (!s || !await this.authorized(s) || !await this.ctx.db.runners.renew(s.id, ws.data.runnerId, ws.data.epoch, v.owned.generation))
                    ...
                    const accepted = await this.ctx.db.runners.complete(v.sessionId, ws.data.runnerId, ws.data.epoch, v.generation, v.error?.slice(-2000) ?? null);
        ...
        close: (ws: ServerWebSocket<RunnerPeer>) => serial(ws, async () => { if (this.peers.get(ws.data.runnerId) === ws) {
            this.peers.delete(ws.data.runnerId);
            await this.ctx.db.runners.disconnect(ws.data.runnerId, ws.data.epoch);
        } }, 'Runner disconnect'),
    };
    async tick() {
        if (this.ticking)
        ...
        this.ticking = true;
        try {
            for (const s of await this.ctx.db.runners.requests()) {
                if (terminal(s.phase))
                    continue;
                if (!await this.authorized(s))
                    await this.ctx.db.runners.stop(s.id, 'Track or runner access ended');
                const current = (await this.ctx.db.runners.request(s.id))!;
                if (current.phase === 'Stopping' || current.phase === 'Reconciling' || (current.phase !== 'Queued' && current.leaseUntil <= Date.now()))
                    ...
                    const actual = await this.engine.info(s.id);
                    if (actual && terminal(actual.phase))
                        await this.ctx.db.runners.stop(s.id, actual.error);
                    else if (actual && actual.phase !== 'Awaiting runner') {
                        current.phase = actual.phase;
                        await this.ctx.db.runners.save(current);
                    }
                }
            }
            await this.ctx.db.runners.reconcile();
            if (await this.engine.busy())
                return;
            for (const [id, ws] of this.peers) {
                if (!ws.data.hello || ws.data.owned)
                    continue;
                const s = await this.ctx.db.runners.assign(id, ws.data.epoch);
                if (!s)
                    continue;
                const code = randomToken(), hash = await sha256(code);
                if (this.peers.get(id) !== ws || !await this.current(s.id, s.generation, s.epoch))
                    continue;
                try {
                    this.engine.spawn(s, hash);
                }
                catch (error) {
                    await this.ctx.db.runners.stop(s.id, String(error));
                    continue;
                }
