...
export class RunnerCoordinator {
    ...
    private enabled(projectId: string) { const p = this.ctx.db.project(projectId); return !!this.ctx.config.nativePreviewExperiment && !!this.ctx.sprites && !!this.ctx.fountain && p?.repoFullName === 'ravix-hq/ravix-expo-hello' && !p.archivedAt; }
