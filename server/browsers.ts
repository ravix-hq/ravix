...
export class Browsers {
  ...
  async info(projectId: string): Promise<BrowserInfo> {
    const row = await this.ctx.db.browsers.get(projectId);
    return { available: this.available(), session: row ? { id: row.id, profile: "shared", state: row.state, error: row.error } : null,
      checkpoints: row ? await this.ctx.db.browsers.checkpoints(row.id) : [] };
  }
  ...
  async destination(projectId: string) {
    const project = await this.ctx.db.project(projectId);
  ...
  async start(projectId: string, authorize: () => Promise<unknown>) {
    ...
    return this.serial(projectId, async () => {
      await authorize();
      const actual = await this.destination(projectId); await authorize();
      let row = await this.ctx.db.browsers.get(projectId);
      if (!row) {
        ...
        await this.ctx.db.browsers.save(row);
      }
      ...
      try {
        const service = await this.ctx.sprites!.service(actual.sprite, SERVICE); await authorize();
        ...
        row = { ...row, ...actual, state: "stopped", error: null }; await this.ctx.db.browsers.save(row);
        let ready = false;
        ...
        await authorize(); await this.ctx.db.browsers.save({ ...row, state: "ready", error: null });
      } catch (error) {
        await this.ctx.db.browsers.save({ ...row, ...actual, state: "failed", error: error instanceof HttpError ? error.message : "Could not start the shared browser. Check the machine connection and service logs." });
        throw error;
      }
  ...
  async stop(projectId: string) {
    return this.serial(projectId, async () => {
      ...
      if (row?.sprite) {
        ...
        await this.ctx.db.browsers.save({ ...row, state: "stopped", error: null });
      }
    });
  }
  async current(projectId: string, authorize: () => Promise<unknown>) {
  ...
  async execute(projectId: string, body: Record<string, unknown>, actor: BrowserActor, authorize: () => Promise<unknown>) {
    return this.serial(projectId, async () => {
      ...
      if ((this.holds.get(row.id) ?? 0) < Date.now() - 30000) {
        await this.ctx.sprites!.activity(row.sprite!, SERVICE); this.holds.set(row.id, Date.now()); await authorize();
      }
      const result = await this.transport(row, { ...body, actor });
      await authorize(); return result;
    });
  }
  async checkpoint(projectId: string, label: string, actor: BrowserActor, authorize: () => Promise<unknown>) {
    return this.serial(projectId, async () => {
      const row = await this.current(projectId, authorize);
      if ((await this.ctx.db.browsers.checkpoints(row.id)).length >= 20) throw new HttpError(409, "browser_checkpoints", "Delete a checkpoint before saving another (limit 20).");
      const payload = await this.transport(row, { action: "checkpoint", actor }); await authorize();
      const payloadEnc = await this.ctx.cipher.encrypt(JSON.stringify(payload)); await authorize();
      ...
      await this.ctx.db.browsers.addCheckpoint(cp, (await this.ctx.db.project(projectId))!.userId, payloadEnc);
      return cp;
    });
  }
  async restore(projectId: string, checkpointId: string, ownerId: string, actor: BrowserActor, authorize: () => Promise<unknown>) {
    return this.serial(projectId, async () => {
      const row = await this.current(projectId, authorize);
      const cp = await this.ctx.db.browsers.checkpoint(checkpointId);
      if (!cp || cp.ownerId !== ownerId || (await this.ctx.db.project(projectId))?.userId !== ownerId) throw new HttpError(404, "not_found", "No such browser checkpoint.");
      const checkpoint = JSON.parse(await this.ctx.cipher.decrypt(cp.payloadEnc)); await authorize();
...
export async function browserRoute(ctx: AppContext, req: Request, trackId: string, action?: string) {
  ...
  const authorize = async () => {
    if (!await ctx.db.sessionUser(sessionHash)) throw new HttpError(401, "unauthenticated", "Sign in again to use the browser.");
    const access = await trackAccess(ctx, user, trackId); if (access.track.closedAt) throw new HttpError(409, "track_closed", "This track is closed."); return access;
  };
  const access = await authorize(), manager = browsers(ctx), projectId = access.project.id;
  if (!action) return json({ data: await manager.info(projectId) }, 200, { "cache-control": "no-store" });
  ...
  if (action === "stop") { requireOwner(access.role, "stop the shared browser"); await manager.stop(projectId); return json({ data: await manager.info(projectId) }); }
  ...
  if (action === "delete-checkpoint") {
    ...
    const cp = await ctx.db.browsers.checkpoint(String(body.checkpointId));
    if (!cp || cp.sessionId !== (await ctx.db.browsers.get(projectId))?.id) throw new HttpError(404, "not_found", "No such browser checkpoint.");
    await ctx.db.browsers.deleteCheckpoint(cp.id); return json({ data: await manager.info(projectId) });
  }
