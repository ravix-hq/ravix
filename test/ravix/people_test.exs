defmodule Ravix.PeopleTest do
  ...
  describe "track membership rows" do
    ...
    test "an invitation reaches one track and stops there", ctx do
      ...
      assert People.member?(ctx.shared.id, ctx.guest.id)
      ...
      refute People.member?(ctx.private.id, ctx.guest.id)
      assert ids(People.member_tracks(ctx.guest.id)) == [ctx.shared.id]
    end

    test "somebody who was never invited is in nothing", ctx do
      refute People.member?(ctx.shared.id, ctx.other.id)
      assert People.member_tracks(ctx.other.id) == []
    end

    test "removing somebody removes them, and only them", ctx do
      People.add_member(ctx.shared.id, ctx.guest.id, "owner")
      People.add_member(ctx.shared.id, ctx.other.id, "owner")
      People.remove_member(ctx.shared.id, ctx.guest.id)

      refute People.member?(ctx.shared.id, ctx.guest.id)
      assert People.member?(ctx.shared.id, ctx.other.id)
    end
    ...
    test "members are listed oldest invitation first", ctx do
      ...
      assert logins(People.members_of(ctx.shared.id)) == ["cy", "bo"]
    end
    ...
    test "closing a track takes its membership out of circulation", ctx do
      ...
      assert People.member_tracks(ctx.guest.id) == []
      assert People.member?(ctx.shared.id, ctx.guest.id)
    end
  end
  ...
  describe "track invitations" do
    ...
    test "an invitation waits on the GitHub account, not on a row we hold", ctx do
      ...
      assert Enum.map(People.invites_of(ctx.shared.id), & &1.login) == ["dana"]
      # Nobody has joined: an invitation is a promise, not access.
      assert People.members_of(ctx.shared.id) == []

      dana = insert_user(github_id: "9001", login: "dana")
      joined = People.claim_invites(dana.id, "9001")

      assert ids(joined.tracks) == [ctx.shared.id]
      assert joined.projects == []
      assert People.member?(ctx.shared.id, dana.id)
      assert People.invites_of(ctx.shared.id) == []
    end
    ...
    test "re-inviting the same account refreshes the name it is shown with", ctx do
      ...
      assert [%{github_id: "9001", login: "dana-codes", avatar_url: "https://a/9001"}] =
               People.invites_of(ctx.shared.id)
    end
    ...
    test "a renamed login still finds them; a recycled one does not", ctx do
      ...
      assert length(People.claim_invites(renamed.id, "9001").tracks) == 1
      ...
      assert People.claim_invites(impostor.id, "9999").tracks == []
      refute People.member?(ctx.shared.id, impostor.id)
    end
    ...
    test "an invitation to a track that closed meanwhile grants nothing", ctx do
      ...
      dana = insert_user(github_id: "9001", login: "dana")
      assert People.claim_invites(dana.id, "9001").tracks == []
      refute People.member?(ctx.shared.id, dana.id)
    ...
    test "an invitation can be withdrawn before it is taken up", ctx do
      ...
      assert People.remove_invite_by_login(ctx.shared.id, "DANA")
      assert People.invites_of(ctx.shared.id) == []
      refute People.remove_invite_by_login(ctx.shared.id, "dana")
    end
  end
  ...
  describe "track link rows" do
    ...
    test "minting a link revokes the one that was out", ctx do
      People.put_link(ctx.shared.id, "hash-one", "owner", 60_000)
      People.put_link(ctx.shared.id, "hash-two", "owner", 60_000)

      # One row per track, so there is no "old link" left to work.
      assert People.track_for_link("hash-one") == nil
      assert People.track_for_link("hash-two").id == ctx.shared.id
    end

    test "a link stops working when revoked, expired, or its track closes", ctx do
      People.put_link(ctx.shared.id, "live", "owner", 60_000)
      assert People.track_for_link("live").id == ctx.shared.id
      assert %{created_at: %DateTime{}, expires_at: %DateTime{}} = People.link_of(ctx.shared.id)
      People.drop_link(ctx.shared.id)
      assert People.track_for_link("live") == nil
      assert People.link_of(ctx.shared.id) == nil

      People.put_link(ctx.shared.id, "stale", "owner", -1)
      assert People.track_for_link("stale") == nil

      People.put_link(ctx.private.id, "doomed", "owner", 60_000)
      close_track(ctx.private)
      assert People.track_for_link("doomed") == nil
    end

    test "an unknown token opens nothing" do
      assert People.track_for_link("never-minted") == nil
    end
  end
  ...
  describe "project membership rows" do
    ...
    test "a project membership is not a track membership", ctx do
      ...
      refute People.member?(ctx.shared.id, ctx.guest.id)
      assert People.member_tracks(ctx.guest.id) == []
      assert ids(People.member_projects(ctx.guest.id)) == [ctx.project.id]
    end
    ...
    test "the wider grant replaces the narrower ones, so nobody holds two", ctx do
      ...
      People.remove_project_member(ctx.project.id, ctx.guest.id)
      assert People.member_projects(ctx.guest.id) == []
      assert People.member_tracks(ctx.guest.id) == []
    ...
    test "removing somebody from a project revokes their grants on every open track", ctx do
      ...
      close_track(closed)
      People.add_project_member(ctx.project.id, guest_id, "owner")
      ...
      People.remove_project_member(ctx.project.id, guest_id)
    ...
    test "the owner is never a member of their own project", ctx do
      ...
      assert People.claim_invites(ctx.owner.id, "1").projects == []
      refute People.project_member?(ctx.project.id, ctx.owner.id)
      assert People.project_members_of(ctx.project.id) == []
      assert People.project_invites_of(ctx.project.id) == []
    end
    ...
    test "an archived project is not somewhere to arrive", ctx do
      ...
      dana = insert_user(github_id: "9001", login: "dana")
      assert People.claim_invites(dana.id, "9001").projects == []
      refute People.project_member?(ctx.project.id, dana.id)
      assert People.project_invites_of(ctx.project.id) == []
    end
  end
  ...
  describe "project invitations" do
    ...
    test "a project invitation waits on the GitHub account, like a track's", ctx do
      ...
      assert Enum.map(People.project_invites_of(ctx.project.id), & &1.login) == ["dana"]
      assert People.project_members_of(ctx.project.id) == []
      ...
      assert People.project_member?(ctx.project.id, dana.id)
      assert People.project_invites_of(ctx.project.id) == []
    end
    ...
    test "a project invitation supersedes a pending track one on the same project", ctx do
      ...
      assert People.invites_of(ctx.shared.id) == []
      assert Enum.map(People.project_invites_of(ctx.project.id), & &1.login) == ["dana"]
      assert People.has_project_invite?(ctx.project.id, "9001")

      # Only on that project, though: an invitation to a track of somebody
      # else's machine is a different decision entirely.
      refute People.has_project_invite?("nope", "9001")
      assert Enum.map(People.invites_of(elsewhere.id), & &1.login) == ["dana"]
    end

    test "a project invitation can be withdrawn before it is taken up", ctx do
      insert_project_invite(ctx.project, github_id: "9001", login: "dana")

      assert People.remove_project_invite_by_login(ctx.project.id, "DANA")
      assert People.project_invites_of(ctx.project.id) == []
      refute People.remove_project_invite_by_login(ctx.project.id, "dana")
    end
  end
  ...
  describe "project link rows" do
    ...
    test "a project link behaves like a track's, and the two do not collide", ctx do
      People.put_project_link(ctx.project.id, "p-one", "owner", 60_000)
      People.put_project_link(ctx.project.id, "p-two", "owner", 60_000)
      # One row per project, so minting is the revoke here too.
      assert People.project_for_link("p-one") == nil
      assert People.project_for_link("p-two").id == ctx.project.id
      assert %{created_at: %DateTime{}} = People.project_link_of(ctx.project.id)

      People.drop_project_link(ctx.project.id)
      assert People.project_for_link("p-two") == nil
      assert People.project_link_of(ctx.project.id) == nil

      People.put_project_link(ctx.project.id, "stale", "owner", -1)
      assert People.project_for_link("stale") == nil
      ...
      # whole machine.
      People.put_link(ctx.shared.id, "t-token", "owner", 60_000)
      People.put_project_link(ctx.project.id, "p-token", "owner", 60_000)
      assert People.project_for_link("t-token") == nil
      assert People.track_for_link("p-token") == nil
    end

    test "an archived project's link opens nothing, and its people go with it", ctx do
      People.add_project_member(ctx.project.id, ctx.guest.id, "owner")
      People.put_project_link(ctx.project.id, "live", "owner", 60_000)

      archive_project(ctx.project)

      assert People.project_for_link("live") == nil
  ...
  describe "read markers" do
    ...
    test "a read is per person per track, and the latest look wins", ctx do
      assert People.last_read_of(ctx.shared.id, ctx.guest.id) == nil

      first = ~U[2026-03-01 10:00:00.000000Z]
      later = ~U[2026-03-01 11:00:00.000000Z]
      People.mark_read(ctx.shared.id, ctx.guest.id, first)
      People.mark_read(ctx.shared.id, ctx.guest.id, later)
      People.mark_read(ctx.private.id, ctx.guest.id, first)
      People.mark_read(ctx.shared.id, ctx.owner.id, first)

      assert People.last_read_of(ctx.shared.id, ctx.guest.id) == later
      assert People.last_read_of(ctx.shared.id, ctx.owner.id) == first

      assert People.reads_of(ctx.guest.id, ctx.project.id) == %{
               ctx.shared.id => later,
               ctx.private.id => first
             }

      # Another project's tracks are not in the answer.
      assert People.reads_of(ctx.guest.id, insert_project().id) == %{}
    end

    test "marking read defaults to now", ctx do
      People.mark_read(ctx.shared.id, ctx.guest.id)
      assert %DateTime{} = seen = People.last_read_of(ctx.shared.id, ctx.guest.id)
  ...
  describe "add/3" do
    ...
    test "somebody who has not signed in is invited on GitHub's account", ctx do
      ...
      assert [%{github_id: "9001", login: "dana"}] = People.invites_of(ctx.shared.id)
      assert People.members_of(ctx.shared.id) == []
    end
  ...
  describe "remove/3" do
    ...
    test "the owner removes a member, case-insensitively and with or without the @", ctx do
      ...
      %{id: guest_id} = ctx.guest
      People.add_member(track_id, guest_id, "owner")
      People.add_member(track_id, ctx.other.id, "owner")
      ...
      refute People.member?(track_id, guest_id)
    end

    test "a member may leave, and gets no list back", ctx do
      %{id: track_id} = ctx.shared
      People.add_member(track_id, ctx.guest.id, "owner")
      Ravix.Hub.subscribe(ctx.project.id)
    ...
    test "a member may not remove somebody else, or withdraw an invitation", ctx do
      ...
      assert length(People.invites_of(ctx.shared.id)) == 1
      assert People.member?(ctx.shared.id, ctx.other.id)
    end
  ...
  describe "list_project/2, add_project/3 and remove_project/3" do
    ...
    test "removing from the project takes every track with it", ctx do
      %{id: guest_id} = ctx.guest
      People.add_member(ctx.private.id, guest_id, "owner")
      People.add_project_member(ctx.project.id, guest_id, "owner")
      Ravix.Hub.subscribe(ctx.project.id)
    ...
    test "withdrawing a project invitation, and leaving", ctx do
      ...
      refute People.project_member?(ctx.project.id, ctx.guest.id)
