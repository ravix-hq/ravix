# Contexts other agents are building, stubbed here. Each is copied for Mimic
# (idempotent: test_helper.exs already copies the ones that exist), and a
# stand-in with the contract's signatures is defined when the real module has
# not landed yet, so this file runs on its own. Mimic copies from object code,
# so a stand-in is written as a beam on a scratch code path.
stand_in_path = Path.join(System.tmp_dir!(), "ravix-stand-ins-#{System.pid()}")
System.at_exit(fn _ -> File.rm_rf(stand_in_path) end)

for {mod, stand_in} <- [
      {Ravix.Tracks,
       quote do
         def close_all_for_rebuild(_project, _reason), do: :ok
       end},
      {Ravix.Previews,
       quote do
         def retire_project(_project_id), do: :ok
       end},
      {Ravix.MachineCache,
       quote do
         def conversations(_client, _project, _opts), do: {:ok, []}
         def forget_project(_project_id), do: :ok
       end}
    ] do
  unless Code.ensure_loaded?(mod) do
    # Mimic reads the abstract code, which only a debug_info build carries.
    previous = Code.get_compiler_option(:debug_info)
    Code.put_compiler_option(:debug_info, true)
    {:module, ^mod, beam, _} = Module.create(mod, stand_in, Macro.Env.location(__ENV__))
    Code.put_compiler_option(:debug_info, previous)
    File.mkdir_p!(stand_in_path)
    File.write!(Path.join(stand_in_path, "#{mod}.beam"), beam)
    Code.prepend_path(stand_in_path)
  end
